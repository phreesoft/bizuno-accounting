<?php
/*
 * Inventory - Prices by fixed discount method
 *
 * NOTICE OF LICENSE
 ...
 * @filesource /controllers/inventory/prices/fxdDiscount.php
 */
...
class fxdDiscount extends inventoryPrices
{
    public $moduleID = 'inventory';
    public $methodDir= 'prices';
    public $code     = 'fxdDiscount';
    ...
    public function priceRender(&$layout=[], $settings=[])
    {
        ...
        $layout['fields']['item'    .$this->code] = ['order'=> 1,'attr'=>['type'=>'hidden']];
        ...
        $keys = ['id'.$this->code,'item'.$this->code,'title'.$this->code,'default'.$this->code,'contact_id'.$this->code,'ref_id'.$this->code,'currency'.$this->code];
        $jsHead = "
...
function preSubmitPrices() {
    ...
    /**
     * This method saves the form contents for fxdDiscount pricing into the database, it is called from method: inventoryPrices:save
     * @param string $layout
    ...
    public function priceSave(&$layout=[])
    {
        ...
        foreach ($levels as $level) {
            if (!empty($level['source']) && !empty($level['qty'])) {
    ...
    public function priceQuote(&$prices, $args)
    {
        ...
        msgDebug("\nEntering prices/fxdDiscount/priceQuote and sheets = ".print_r($sheets, true));
        if (empty($sheets) || $prices['method']=='byContact') { return; }
        foreach ($sheets as $sheet) {
            if ($sheet['id']==$args['cSheet']) {
                // Only one allowed or output will be confused, just use the first one
                $settings  = json_decode($sheets[0]['settings'], true);
                $prices['discount'] = ['title'=>$settings['title'], 'encoded'=>$settings['attr']];
                break;
            }
        }
    }
